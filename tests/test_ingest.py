...
from app.store import STORE # <--- Only import STORE
...
def test_ingest_rejects_tenant_id_in_payload():
    ...
    assert r.json()["reason_code"] == "TENANT_FIELD_FORBIDDEN"
...
def test_intern_cannot_ingest_admin_classification():
    ...
    assert r.json()["reason_code"] == "CLASSIFICATION_FORBIDDEN"
...
def test_ingest_derives_tenant_from_principal_and_stores_doc():
    ...
    # FIX: Use the STORE singleton method
...
def test_ingest_invalid_classification_is_400():
    ...
    assert r.status_code == 400
    assert r.json()["reason_code"] == "INVALID_CLASSIFICATION"